...
const Egpvm1ApmRaasConfigOpt20 = () => {
  const navigate = useNavigate();

  const mainHeaders = [{ title: 'APM 4 EGPWM OPTIONS' }, { title: 'RNIC 2', customSpan: 1 }];
  ...
  return (
    <DefaultLayout showNavBar navBarButtons={navBarButtons} footer={<DataFooter left={'344616'} right={['select prev to go back', 'Select done to continue']} />}>
      <DataHeader left={'eg pwm apm raas config options'} right={'20/20'} />
      <CustomTable data={data} headers={mainHeaders} />
    </DefaultLayout>
  );
};

export default Egpvm1ApmRaasConfigOpt20;
