...
const RightOperationalIndication1 = () => {
  const navigate = useNavigate();

  const tableHeader = [{ title: 'PARAMETER', alignLeft: true }, { title: 'CH A' }, { title: 'CH B' }];

  const data = [
    { parameter: 'ENGINE SERIAL #', statuses: ['0 2 1 1', '0 2 1 1'], customSpan: 2, alignCenter: true },
  ...
  return (
    <DefaultLayout showNavBar navBarButtons={navBarButtons} footer={<DataFooter left={'771007'} right={['select prev to go back', 'select fwd to continue']} />}>
      <DataHeader left={'right engine operational indication'} right={'1/4'} />
      <DynamicTable data={data} mainHeaders={tableHeader} />
    </DefaultLayout>
  );
};

export default RightOperationalIndication1;
