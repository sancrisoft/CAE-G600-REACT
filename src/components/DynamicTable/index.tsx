...
const DynamicTable: React.FC<DynamicTableProps> = ({
  ...
  const rows = data.map((item, index) => {
    ...
    const rowClass = `${
      (item.hasBorderBottom || item.hasBorderBottom === undefined) && item.customSpan !== 4 ? styles.Border : ''
    } `
    return (
          ...
          item.statuses.map((status, index) => (
            <td colSpan={customcolspan} key={index} className={!customWidth ? styles.CustomWidth : ''}>
              {typeof status === 'boolean' ? (
                <div
                  className={`${styles.Square} ${getStatusClass(status)} ${
                    item.type ? getypeClass(item.type[index]) : ''
                  } ${item.noData ? styles.NoData : ''}`}
                ></div>
              ) : (
                <p
                  className={`${
                    item.alignRight ? styles.AlignRight : item.alignCenter ? styles.AlignCenter : styles.AlignLeft
                  } `}
                >
                  {status}
                </p>
